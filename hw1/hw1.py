...
def loss_function(preds, labels):
    ...
    #############################
    #### YOUR CODE GOES HERE ####
    preds, labels = preds[:, -1], labels[:, -1] # [B, N, N]
    return tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=preds, axis=-1)
    #############################
...
class MANN(tf.keras.Model):
    ...
    def call(self, input_images, input_labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        batch_size = tf.shape(input_labels)[0]
        new_labels = tf.concat((input_labels[:, :-1], tf.zeros((batch_size, 1, self.num_classes, self.num_classes))), axis=1)
        inp = tf.concat((input_images, new_labels), axis=-1) # [B, K+1, N, 784+N]
        inp = tf.reshape(inp, (batch_size, self.samples_per_class * self.num_classes, 784 + FLAGS.num_classes)) # [B, (K+1)*N, 784+N]
        out = self.layer1(inp)
        out = self.layer2(out)
        out = tf.reshape(out, (batch_size, self.samples_per_class, self.num_classes, self.num_classes))
        #############################
        return out
